...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  // Drop indexes
  await db.execute(sql`DROP INDEX IF EXISTS "jobs_workflow_steps_order_idx";`)
  await db.execute(sql`DROP INDEX IF EXISTS "jobs_workflow_steps_parent_id_idx";`)
  ...
  // Drop table
  await db.execute(sql`DROP TABLE IF EXISTS "jobs_workflow_steps" CASCADE;`)
  
  // Drop columns
  await db.execute(sql`ALTER TABLE "jobs" DROP COLUMN IF EXISTS "workflow_type";`)
  await db.execute(sql`ALTER TABLE "clients" DROP COLUMN IF EXISTS "default_workflow";`)
  
  // Drop enum types
  await db.execute(sql`DROP TYPE IF EXISTS "public"."enum_jobs_workflow_type";`)
  await db.execute(sql`DROP TYPE IF EXISTS "public"."enum_clients_default_workflow";`)
}
